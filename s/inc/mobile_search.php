...
<div class="logo"><a href="<?php echo SYSPATH?>" title="<?php echo $huoduan['sitename']?>"><img src="<?php echo SYSPATH?>images/logo.png" alt="<?php echo $huoduan['sitename']?>" /></a></div>
      ...
      <div class="searchbox">
       <form action="<?php echo SYSPATH?>" method="get"><input align="middle" name="q" class="q" id="kw" value="<?php echo $q?>" maxlength="1000" baiduSug="1" autocomplete="off" x-webkit-speech /><?php if(REWRITE=='1'){?><input name="re" type="hidden" value="1" /><?php }?><input id="btn" class="btn" align="middle" value="搜索" type="submit" />
              </form>
      </div>
<div id="hd_main">
<div class="res">
...
if($listcount>1&& $kill!=1){
  ...
  for($i=0;$i<$listcount;$i++){
	...
	$yurl = $list['data'][$ni]['blink'];
	  ...
	  if($kurl!=1){
	?>
<div class="g"><h2><?php if($huoduan['listnum']==1){?><span class="nums"><?php echo $listnum[$ii]?></span> <?php }?><a href="<?php echo $sourl?>" target="_blank" class="s" rel="nofollow"><?php echo $list['data'][$ni]['title']?></a></h2><div class="std"><?php echo $list['data'][$ni]['des']?></div><span class="a"><?php echo $blink[2]?></span></div>    
    <?php	
	  }
  }
  echo '</div>';
...
<?php }?>
   ...
   <div class="cl10"></div>
    <div class="searchbox">
       <form action="<?php echo SYSPATH?>" method="get"><input align="middle" name="q" class="q" id="kw" value="<?php echo $q?>" maxlength="100" autocomplete="off" x-webkit-speech /><?php if(REWRITE=='1'){?><input name="re" type="hidden" value="1" /><?php }?><input id="btn" class="btn" align="middle" value="搜索" type="submit" />
              </form>
      </div>
<div class="cl10"></div>
   <?php
